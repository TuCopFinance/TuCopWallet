...
describe('isSameNftContract', () => {
  it('returns true when NFTs have the same network ID and contract address', () => {
    const nft1 = { networkId: NetworkId['celo-mainnet'], contractAddress: '0xTEST' }
    const nft2 = { networkId: NetworkId['celo-mainnet'], contractAddress: '0xTEST' }
  ...
  it('returns false when NFTs have different networkIds', () => {
    const nft1 = { networkId: NetworkId['celo-mainnet'], contractAddress: '0xTEST' }
    const nft2 = { networkId: NetworkId['ethereum-mainnet'], contractAddress: '0xTEST' }
    expect(isSameNftContract(nft1, nft2)).toBe(false)
  })

  it('returns false when NFTs have different contract addresses', () => {
    const nft1 = { networkId: NetworkId['celo-mainnet'], contractAddress: '0xTEST' }
    const nft2 = { networkId: NetworkId['celo-mainnet'], contractAddress: '0xOTHER' }
    expect(isSameNftContract(nft1, nft2)).toBe(false)
  })
})
