...
describe('Price History Selectors', () => {
  describe('tokenPriceHistorySelector', () => {
    it('returns the right tokens', () => {
      const tokenPriceHistory = priceHistoryPricesSelector(state, 'celo-mainnet:0xusd')
      expect(tokenPriceHistory).toStrictEqual(celoPrices)
    })

    it('avoids unnecessary recomputation', () => {
      const tokenPriceHistory = priceHistoryPricesSelector(state, 'celo-mainnet:0xusd')
      const tokenPriceHistory2 = priceHistoryPricesSelector(state, 'celo-mainnet:0xusd')
  ...
  describe('tokenPriceHistoryStatusSelector', () => {
    it('returns the right status', () => {
      const tokenPriceHistoryStatus = priceHistoryStatusSelector(state, 'celo-mainnet:0xusd')
      expect(tokenPriceHistoryStatus).toStrictEqual('success')
    })

    it('avoids unnecessary recomputation', () => {
      const tokenPriceHistoryStatus = priceHistoryStatusSelector(state, 'celo-mainnet:0xusd')
      const tokenPriceHistoryStatus2 = priceHistoryStatusSelector(state, 'celo-mainnet:0xusd')
